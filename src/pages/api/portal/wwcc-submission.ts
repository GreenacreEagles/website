...
import { deleteR2Object, getPrivateMediaBucket, getRuntimeEnv, getUploadedFile, logPrivateUploadAudit, validatePrivateFile, wwccDocumentObjectKey } from "@lib/media";
...
export const POST: APIRoute = async (context) => {
  ...
  const limit=await consumeRateLimit({supabase:session.supabase,limitClass:"uploads",key:rateLimitKey([session.user.id,clientIp(context.request)])});
  if(!limit.allowed) return context.redirect(rateLimitRedirect(back,limit),303);
