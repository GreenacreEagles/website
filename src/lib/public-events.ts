...
export async function getPublicEvents(client:SupabaseClient<Database>,context:{locals?:any},limit?:number):Promise<PublicEventSummary[]> {
  const boundedLimit = clampLimit(limit, PAGE_BOUNDS.events);
  const {data,error}=await (client as any).from("club_events")
    ...
    .eq("status","active").eq("visibility","public").gt("starts_at",new Date().toISOString()).order("starts_at").limit(boundedLimit);
