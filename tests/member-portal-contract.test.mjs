...
test("child navigation is restricted to operational member pages", async () => { const source=await read("src/layouts/PortalLayout.astro"); assert.match(source,/isChildAccount/); assert.doesNotMatch(source,/Family groups/); assert.match(source,/Wallet/); assert.match(source,/Canteen Staff/); });
test("wallet hides internal id and QR is disclosed on request", async () => { const source=await read("src/pages/portal/vouchers.astro"); assert.doesNotMatch(source,/>Wallet ID</); assert.match(source,/Show wallet QR code/); assert.doesNotMatch(source,/wallet.displayCode/); assert.match(source,/PhoneShare/); assert.match(source,/ticketShareMessage/); assert.match(source,/voucherShareMessage/); });
