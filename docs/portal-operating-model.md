...
The commerce source phases add canteen catalogue management, stock-aware canteen ordering, staff order transitions, payment marking, pickup codes, automatic wallet voucher issuance for paid voucher products, voucher QR scanning and reversal, wallet account creation, manual top-up settlement, controlled wallet adjustments, ledger reversal, merchandise catalogue management, stock-backed merchandise checkout and merchandise order operations. Full production completion still requires applying the latest migration, regenerating database types, external payment-provider integration, webhook idempotency, R2 upload endpoints, event ticket QR scanning, public store checkout and broader automated coverage.
